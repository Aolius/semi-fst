    ...
    def read(self, model_path):
        with open(model_path, 'r', encoding="utf-8") as f:
            for line in f.readlines():
                ...
                # set() will cause a random order of dictionary values, here we use sort to eliminate such randomness
                unordered_list = list(set(self.dict[key]))
                unordered_list.sort(key=self.dict[key].index)
                self.dict[key] = unordered_list
